...
export default function ManagerLoginPage() {
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    const result = await signIn("manager-credentials", {
      redirect: false,
      username,
      password,
    });
    ...
    if (result?.error) {
      setError("Invalid Pantry ID or manager password. Please try again.");
      return;
    }
    const raw = searchParams.get("callbackUrl");
    const target = raw && raw.startsWith("/") && raw !== "/" ? raw : "/manager";
    ...
    <div className="flex min-h-screen items-center justify-center bg-zinc-50 dark:bg-black">
      <div className="w-full max-w-md rounded-2xl bg-white p-8 shadow-lg dark:bg-zinc-900">
        ...
        <p className="mb-8 text-center text-sm text-zinc-500 dark:text-zinc-400">
          Sign in with your Pantry ID and manager password to access Manager View.
        </p>
